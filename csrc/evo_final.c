...
static double eval_w(Game *g, Color c, const double *W) {
    ...
    for(int si=0;si<s->settlement_count[idx];si++){int node=s->settlements[idx][si];
        { if(node<0||node>=NUM_NODES)continue; } for(int ti=0;ti<map->adjacent_tiles_count[node];ti++){int t2=map->adjacent_tiles[node][ti]; if(t2<0||t2>=NUM_LAND_TILES)continue;
    ...
    for(int si=0;si<s->settlement_count[idx];si++){int node=s->settlements[idx][si];
        { if(node<0||node>=NUM_NODES)continue; } for(int ti=0;ti<map->adjacent_tiles_count[node];ti++){int t2=map->adjacent_tiles[node][ti]; if(t2<0||t2>=NUM_LAND_TILES)continue;if(!ts[t2]){ts[t2]=1;nt++;}}}
    for(int ci=0;ci<s->city_count[idx];ci++){int node=s->cities[idx][ci];
        { if(node<0||node>=NUM_NODES)continue; } for(int ti=0;ti<map->adjacent_tiles_count[node];ti++){int t2=map->adjacent_tiles[node][ti]; if(t2<0||t2>=NUM_LAND_TILES)continue;if(!ts[t2]){ts[t2]=1;nt++;}}}
