...
static void player_freqdeck_add(State *s, int idx, const int fd[5]) {
    for (int r = 0; r < 5; r++)
        s->player_state[idx][PS_RESOURCE_IN_HAND(r)] += fd[r];
}

static void player_freqdeck_sub(State *s, int idx, const int fd[5]) {
    for (int r = 0; r < 5; r++)
        s->player_state[idx][PS_RESOURCE_IN_HAND(r)] -= fd[r];
}
...
static void reset_trading_state(State *s) {
...
static void do_roll(State *s, Action a, RngState *rng) {
    ...
    if (number == 7) {
        ...
        for (int i = 0; i < s->num_players; i++) {
            ...
            int dc = (num_cards > s->discard_limit) ? (num_cards + 1) / 2 : 0;
