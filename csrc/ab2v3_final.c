...
int main(void){
    ...
    printf("=== AB:3 (1 seat) vs AB:2 (3 seats): %dk games, %d threads ===\n",GAMES/1000,NT);
    printf("Thread stack: 4MB (8x default)\n\n");

    uint64_t seed=(uint64_t)time(NULL)*1000003ULL;
    pthread_t th[NT];WA wa[NT];
    pthread_attr_t attr;
    pthread_attr_init(&attr);
    pthread_attr_setstacksize(&attr, 2*1024*1024); /* 4MB stack per thread */

    for(int i=0;i<NT;i++){wa[i].tid=i;wa[i].n=GPT;wa[i].sb=seed;}
    struct timespec T0,T1;clock_gettime(CLOCK_MONOTONIC,&T0);
    for(int i=0;i<NT;i++) pthread_create(&th[i],&attr,worker,&wa[i]);
