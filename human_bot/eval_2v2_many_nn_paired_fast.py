"""Paired batch 2v2 no-search evaluation for many candidate NN binaries.

For each seed/rotation this runs two games:
  1. candidate controls the A seats, opponent controls the B seats
  2. opponent controls the A seats, candidate controls the B seats

The output keeps the same keys as ``eval_2v2_many_nn_fast`` where practical:
``a_wins`` means candidate wins and ``a_winrate`` means candidate win rate.
This makes it usable as a drop-in, less seat-biased proxy for evolution.
"""
...
def _play_one(job):
    cand_idx, game_idx, seed, a_seats, b_seats, candidate_side = job
    ...
    opp_ptr = _G["b_ptr"]
    candidate_seats = a_seats if candidate_side == 0 else b_seats
    ...
    while not game.is_terminal() and game.turn_number < 500:
        ...
        if cp in a_seats:
            model_ptr = cand_ptr if candidate_side == 0 else opp_ptr
        else:
            model_ptr = opp_ptr if candidate_side == 0 else cand_ptr
        game.step(nn_argmax(legal, model_ptr))
    ...
    return {
        ...
        "winner": winner,
        "candidate_won": winner in candidate_seats if winner is not None else False,
    }
...
def run(a_weights, b_weights, games, workers, seed_base):
    ...
    for ci in range(len(a_weights)):
        for gi in range(games):
            ...
            seed = seed_base + gi
            jobs.append((ci, gi, seed, a_seats, b_seats, 0))
            jobs.append((ci, gi, seed, a_seats, b_seats, 1))
    ...
    for ci, path in enumerate(a_weights):
        ...
        as_a = [r for r in rows if r["candidate_side"] == 0]
        as_b = [r for r in rows if r["candidate_side"] == 1]
        results.append({
            "candidate": ci,
            ...
            "candidate_wins_as_a": sum(1 for r in as_a if r["candidate_won"]),
            "candidate_wins_as_b": sum(1 for r in as_b if r["candidate_won"]),
        })
    return {
        "games_per_side": games,
...
def main():
    ...
    if args.json:
        ...
        print(f"=== eval_2v2_many_nn_paired_fast: {res['candidates']} candidates ===")
